...
export default function TimedQuizPage() {
  ...
  //   const cards = source === "custom" ? getCustomFlashcards() : flashcards;
  //   const unique = [...new Set(cards.map(card => card.set || card.category || "Uncategorized"))];
  ...
  useEffect(() => {
    const loadFlashcards = async () => {
      ...
      const filtered = selectedCategory === "Default"
        ? cards
        : cards.filter(card =>
            (card.set || card.category)?.toLowerCase() === selectedCategory.toLowerCase()
          );
    ...
    return (
      <div className="p-4 max-w-2xl mx-auto">
        ...
        <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block font-semibold mb-1">Quiz Duration (sec)</label>
            <input
              type="number"
              value={duration}
  ...
  // Prepare limited flashcards after user starts
